...
void Ov7670_Gpio_Init()
{
    for(int i=0;i<8;i++)
    {
        ...
        gpio_set_pull(DataLine[i],PULL_UP);
    }
}
/***********************
 * 功能：写OV7670的寄存器
...
void Ov7670_Config_Window(uint32_t start_x,uint32_t start_y,
                         uint32_t width,uint32_t height)
{
    ...
    end_x = (start_x+width);
    //v*2必须
    end_y = (start_y+height+height);
    Read_Ov7670_Reg(0x03,&temp_reg1); //VREF
    temp_reg1 &=0xf0;
    Read_Ov7670_Reg(0x32,&temp_reg2); //HREF
    temp_reg2 &=0xc0;

    //Horizontal
    temp = temp_reg2|((end_x&0x7)<<3)|(start_x&0x7);
    Write_Ov7670_Reg(0x32,temp);
    //0x17 OutPut fromat - Horizontal Frame
    temp = (start_x&0x7F8)>>3;
    Write_Ov7670_Reg(0x17,temp);
    temp = (end_x&0x7F8)>>3;
    Write_Ov7670_Reg(0x18,temp);

    //Vertical
    temp = temp_reg1|(end_y&0x3<<2)|(start_y&0x3);
...
uint8_t Fps15_config()  
{
    if(0==Write_Ov7670_Reg(0x13,0xe7))
    {
        return 0;
    }
    if(0==Write_Ov7670_Reg(0x9d,0xa5))
    {
        return 0;
    }
    if(0==Write_Ov7670_Reg(0x9e,0x44))
    {
        return 0;
    }
    if(0==Write_Ov7670_Reg(0xa5,0x06))
    {
        return 0;
    }
    if(0==Write_Ov7670_Reg(0xab,0x07))
    {
        return 0;
    }
    if(0==Write_Ov7670_Reg(0x3b,0xa1))
    {
        return 0;
    }
    return 1;
...
uint8_t Ov7670_init()
{
  ...
  //  Ov7670_Config_Window(272,12,320,240);//设置像素为320*240
    ...
    if(0==Fps15_config())
    {
        printf("[error] fps15 config failed\n");
        return 0;
    }
    */
   // Set_Ov7670_Reg();
...
int main(int argc,char **argv)
{
     ...
     //   CLK_Init_Off();
        ...
        while(height--)
        {
            ...
            while(temp--){
                ...
                //  printf("[info]:wait plck high\n");
                //wait for low
                uint8_t data=0x00;
                for(uint8_t i=0;i<8;i++)
                {
                    data |=(get_gpio_level(DataLine[i])<<i);
                }
                fwrite(&data,sizeof(data),1,fp);
                printf("temp: %d data is %x\n",temp,data);
                while(!Get_PLCK_state());
                  //  printf("[info]:wait PLCK low\n");
                //wait for high
            }
            //printf("[info]:one row finished\n");
        }
        fclose(fp);
        if(captureNum>10){
            break;
    ...
    /*    while(pix_Count<76800)
        ...
        //    XCLK_L();
            //delay_ms(1);
        //    XCLK_H();
        //    printf("[info]:ready for get data\n");
         //   delay_ms(1000);
            for(uint8_t i=0;i<8;i++)
            {
                value_temp |= get_gpio_level(DataLine[i])>>i;
            }
            printf("[info]:the value temp is %X",value_temp);
            value_temp = 0x0000;
            pix_Count++;
        }    
        */
